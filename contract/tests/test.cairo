...
fn deploy_token(recipient: ContractAddress) -> IERC20Dispatcher {
    ...
    let address = contract.deploy_at(@calldata, STRK_ADDRESS.try_into().unwrap()).expect('unable to deploy mockstrk');
...
fn test_claim_one_root() {
    ...
    let claimee_addr = CLAIMEE_1.try_into().unwrap();
    start_prank(CheatTarget::One(contract.contract_address), CLAIMEE_1.try_into().unwrap()); // does not actually matter, TODO check if business logic approves
    let proof = array![0x2f582855ca3f9bb074b939b1670554bd01334b0bc9fe95ed7577295db1086b, 0xe5c5a70b996a566aa28559817bac9a79a6575090abaa9509f606e1b25dd98];
    contract.claim(claimee_addr, 0x88, proof.span());
}
...
fn test_compute_root() {
    ...
    assert(root == 0x45aa6b933e7b76e85c77fc12b2cc58c22ba87b76fb7595bd315fb3ede730dfe, 'roots dont match');
}
